...
void insertSM(int table, char *name) {
    ...
    if (currentSymbol == NULL) {
		...
		else 
		{
			...
			separators_table[hashval] = (SymbolInfo_SM**)newSymbol_SM;
		}
...
void dumpIDF() {
    printf("+======================================================================================================================================================================================================+\n");
    printf("| %-5s | %-20s | %-5s | %-12s | %-12s | %-15s | %-12s | %-9s | %-11s | %-41s | %-24s |\n",
            ...
            while (symbol != NULL)
			{
				...
				if (symbol->inf_type == INT_TYPE || symbol->smb_type == INT_TYPE) {
					...
					if (symbol->smb_type != ARRAY_TYPE && symbol->smb_type != FUNCTION_TYPE) 	
						...
						printf(" %-15s |", "NoN");
				} else if (symbol->smb_type == FUNCTION_TYPE) 
					printf(" %-15s |", symbol->val);
				else {
...
char *get_val(char *name,int scopeid) {
	...
	{
		setColor(255, 0, 0);
		printf("\n\nSymbol not found, %s is undeclared\n", name);
		setColor(255, 255, 255);
		return NULL;
	}
